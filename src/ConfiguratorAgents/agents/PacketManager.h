...
class PacketManager {
  public:
    enum class ReceivingState{WAITING_HEADER, WAITING_PAYLOAD, WAITING_END, RECEIVED, ERROR};  
    enum class MessageType{DATA = 2, RESPONSE = 3};
    typedef struct{
      MessageType type;
      InputPacketBuffer payload;
    }ReceivedData;
    PacketManager(): _tempInputMessageHeader{5},_tempInputMessageEnd{4}{};
