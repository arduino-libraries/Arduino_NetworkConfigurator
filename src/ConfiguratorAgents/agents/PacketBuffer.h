...
class PacketBuffer{
  ...
  protected:
    bool hasPendingBytes() { return _bytesToTransfer - _bytesTransferred > 0;};
    uint32_t transferredBytes() { return _bytesTransferred;};
    uint32_t pendingBytes() { return _bytesToTransfer - _bytesTransferred;};
    uint32_t bytesToTransfer() { return _bytesToTransfer;};
    void setBytesTransferred(int n) { _bytesTransferred = n;};
    void setBytesToTransfer(int n) { _bytesToTransfer = n;};
...
class OutputPacketBuffer: public PacketBuffer 
{
  public:
    OutputPacketBuffer(){};
    uint32_t len()                       { return bytesToTransfer();};
    uint32_t bytesSent()                 { return transferredBytes();};
    uint32_t bytesToSend()               { return pendingBytes();};
    bool hasBytesToSend()                { return hasPendingBytes();};
    void bytesSent(int n)                { setBytesTransferred(n);};
    void incrementBytesSent(int n)       { setBytesTransferred(transferredBytes() +n);};
    void startProgress()                 { setBytesTransferred(0);};
    void clear()                         { setBytesTransferred(0);};
...
class InputPacketBuffer: public PacketBuffer
{
  public:
    ...
    bool receivedAll()                   { return !hasPendingBytes() && bytesToTransfer() >0 ;};
    uint32_t missingBytes()              { return pendingBytes();};
    uint32_t len()                       { return transferredBytes();};
    void setPayloadLen(uint32_t len)     { setBytesToTransfer(len);};
		void clear()                         { setBytesTransferred(0);};
